 ...
 */
public class ExpressionEclipseProxy {
		...
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			...
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
					...
					for (Object key : keys) {
						org.emftext.term.propositional.expression.resource.expression.util.ExpressionMapUtil.putAndMergeKeys(optionsMap, key, options.get(key));
					}
				} catch (org.eclipse.core.runtime.CoreException ce) {
					new org.emftext.term.propositional.expression.resource.expression.util.ExpressionRuntimeUtil().logError("Exception while getting default options.", ce);
				}
			}
		}
	}
	 ...
	 */
	public org.emftext.term.propositional.expression.resource.expression.mopp.ExpressionResource getResource(org.eclipse.core.resources.IFile file) {
	...
	public void checkEMFValidationConstraints(org.emftext.term.propositional.expression.resource.expression.IExpressionTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (new org.emftext.term.propositional.expression.resource.expression.util.ExpressionRuntimeUtil().isEclipsePlatformRunning()) {
			// The EMF validation framework code throws a NPE if the validation plug-in is not
			...
			if (org.eclipse.emf.validation.internal.EMFModelValidationPlugin.getPlugin() != null) {
				try {
					...
					new org.emftext.term.propositional.expression.resource.expression.util.ExpressionRuntimeUtil().logError("Exception while checking contraints provided by EMF validator classes.", t);
				}
			}
		}
	}
	
	public void addStatus(org.eclipse.core.runtime.IStatus status, org.emftext.term.propositional.expression.resource.expression.IExpressionTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (!status.isMultiStatus() || !hasChildren) {
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.ERROR) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addError(status.getMessage(), org.emftext.term.propositional.expression.resource.expression.ExpressionEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.WARNING) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addWarning(status.getMessage(), org.emftext.term.propositional.expression.resource.expression.ExpressionEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
		}
		...
		if (uri != null && uri.isPlatform()) {
			...
			if (platformResource instanceof org.eclipse.core.resources.IFile) {
				...
				try {
					...
					new org.emftext.term.propositional.expression.resource.expression.util.ExpressionRuntimeUtil().logWarning("Could not determine encoding of platform resource: " + uri.toString(), ce);
				}
			}
		}
		return null;
