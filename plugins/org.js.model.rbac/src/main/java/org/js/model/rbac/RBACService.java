...
public class RBACService {
   ...
   public List<ConfigurationOperation> getAllRoleConfigOperations(Role role) {
      ...
      List<ConfigurationOperation> mergedOperations = mergeOperations(configOps);
      return mergedOperations;
   }

   private List<ConfigurationOperation> mergeOperations(List<ConfigurationOperation> configOps) {
      Map<Feature, List<FeatureConfiguration>> featureConfigs = new HashMap<Feature, List<FeatureConfiguration>>();
      Map<Attribute, List<AttributeConfiguration>> attributeConfigs = new HashMap<Attribute, List<AttributeConfiguration>>();

      // sort config operations
      for (ConfigurationOperation configurationOperation : configOps) {
         if (configurationOperation instanceof AttributeConfiguration) {
            AttributeConfiguration ac = (AttributeConfiguration) configurationOperation;
            Attribute attribute = ac.getAttribute();
            List<AttributeConfiguration> list = attributeConfigs.get(attribute);
            if (list == null) {
               list = new ArrayList<AttributeConfiguration>(2);
               attributeConfigs.put(attribute, list);
            }
            list.add(ac);

         } else {
            FeatureConfiguration fc = (FeatureConfiguration) configurationOperation;
            Feature feature = fc.getFeature();

            List<FeatureConfiguration> list = featureConfigs.get(feature);
            if (list == null) {
               list = new ArrayList<FeatureConfiguration>(2);
      ...
      List<ConfigurationOperation> mergedOps = new ArrayList<>();
      Set<Feature> featureKeySet = featureConfigs.keySet();
      for (Feature feature : featureKeySet) {
         List<FeatureConfiguration> list = featureConfigs.get(feature);
         FeatureConfiguration mergedOp = mergeFeatureConfigOperations(list);
