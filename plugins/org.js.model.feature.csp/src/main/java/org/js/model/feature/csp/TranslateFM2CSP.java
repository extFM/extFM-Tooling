...
public class TranslateFM2CSP {
   ...
   private void transformAttribute(Attribute attribute) {
      ...
      IntegerVariable attributeEnablementVariable = getOrCreateVariable(attribute, false);

      Constraint attributeEnablementConstraint = Choco.eq(featureVariable, attributeEnablementVariable);
      ...
      Constraint checkAttribute = Choco.ifOnlyIf(attributeEnabled, memberConstraint);
      getModel().addConstraint(checkAttribute);

//      // if attribute is disabled, the attribute value is "-1"
//      IntegerVariable attributeValueVariable = getOrCreateVariable(attribute, true);
//      Constraint attributeValueDisabled = Choco.eq(attributeValueVariable, attributeDisabled);
//
//      Constraint attributeDisabled = Choco.eq(attributeEnablementVariable, 0);
//      Constraint notCheckAttribute = Choco.ifOnlyIf(attributeDisabled, attributeValueDisabled);
//
//      getModel().addConstraint(notCheckAttribute);
   }
   ...
   private Constraint getMemberConstraint(Attribute attribute) {
      ...
      if (domain instanceof NumericalDomain) {
         ...
         for (Interval interval : intervals) {
            ...
            Constraint member = Choco.member(attributeValueVariable, lowerBound, upperBound);
            memberConstraints.add(member);
         }
         memberConstraint = Choco.or((Constraint[]) memberConstraints.toArray(new Constraint[memberConstraints.size()]));
   ...
   private void createAbstractAttributeConstraint(Attribute a) {
      log.debug("Create constraint for attribute " + a);
      IntegerVariable feature = getOrCreateVariable(a.getFeature());
      // if feature selected then attribute is enabled
      IntegerExpressionVariable attributeVarSum = createChildAttributeVariable(a);

      Constraint parentSelectedConstraint = Choco.eq(feature, 1);
      Constraint childSumparentSelected = Choco.eq(attributeVarSum, 1);

      // Constraint parentDeSelectedConstraint = Choco.eq(feature, 0);
      // Constraint childSumparentDeSelected = Choco.eq(attributeVarSum, 0);

      Constraint attributeValueParentSelected = Choco.implies(parentSelectedConstraint, childSumparentSelected);
      // Constraint attributeValueParentDeselected = Choco.implies(parentDeSelectedConstraint,
      // childSumparentDeSelected);

      getModel().addConstraint(attributeValueParentSelected);
      // getModel().addConstraint(attributeValueParentDeselected);
   }

   private IntegerExpressionVariable createChildAttributeVariable(Attribute a) {
      int cachedAttributeValue = -1;
      boolean isAttributeSet = FeatureModelHelper.isAttributeValueSet(a);
      if (isAttributeSet) {
         cachedAttributeValue = FeatureModelHelper.getAttributeValueForString(a.getValue(), a);
      }
      ...
      if (domain instanceof NumericalDomain) {
         ...
         List<IntegerVariable> attributeValueVariables = new ArrayList<IntegerVariable>(domainValues.length);
         for (int i = 0; i < domainValues.length; i++) {
            int j = domainValues[i];
            String value = Integer.toString(j);
            String variableId = concatenate(new String[] {
                  attributeValue, featureId, attName, value }, attributeDelimiter);
            // representation of attribute as abstract feature
            IntegerVariable domainvaluevariable = Choco.makeBooleanVar(variableId);
            getModel().addVariable(domainvaluevariable);

            if (isAttributeSet) {
               if (cachedAttributeValue == j) {
                  Constraint domainValueSelected = Choco.eq(domainvaluevariable, 1);
         ...
         IntegerVariable[] attributeVariablesArray = new IntegerVariable[attributeValueVariables.size()];
         attributeVariablesArray = attributeValueVariables.toArray(attributeVariablesArray);
   ...
   private IntegerVariable getOrCreateVariable(Attribute attribute, boolean isValue) {
      ...
      if (integerVariable == null) {
         integerVariable =
            isValue ? createAttributeValueVariable(attribute, attributeId, false) : createAttributeEnableVariable(attributeId);
   ...
   private IntegerVariable createAttributeValueVariable(Attribute attribute, String attributeId, boolean addDisableInteger) {
      ...
      if (attributeDomain instanceof DiscreteDomain) {
         ...
         if (addDisableInteger && lowestBoundofNumericalDomain > attributeDisabled){
   ...
   private IntegerVariable createFeatureVariable(Feature feature) {
      ...
      int minCardinality = (FeatureState.SELECTED.equals(feature.getSelected())) ? 1 : 0;
      int maxCardinality = (FeatureState.DESELECTED.equals(feature.getSelected())) ? 0 : 1;
